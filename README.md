...
---
## eoh_diag (Diagnosis-Driven EoH)

`eoh_diag` is a new method variant that augments EoH with an LLM-based diagnostic layer. After each generation, it summarizes evaluation signals for the current best candidate, asks an LLM for a small targeted patch, validates the patch, and only inserts it if the objective improves.

**What it adds**
- A new method name: `method="eoh_diag"`
- Diagnostic signals for `bp_online` (without changing baseline evaluation)
- Logging of diagnosis inputs/outputs and applied patches under `results/diag_logs/`

**Files changed / added**
- Added: `eoh/src/eoh/methods/eoh_diag/eoh_diag.py`
- Added: `eoh/src/eoh/methods/eoh_diag/eoh_diag_evolution.py`
- Added: `eoh/src/eoh/methods/eoh_diag/eoh_diag_interface_EC.py`
- Added: `examples/bp_online/runEoH_diag.py`
- Updated: `eoh/src/eoh/methods/methods.py` (new entry point)
- Updated: `eoh/src/eoh/utils/getParas.py` (method defaults for `eoh_diag`)
- Updated: `eoh/src/eoh/problems/optimization/bp_online/run.py` (added `evaluate_with_signals`)

**How to run**
- Baseline: `python examples/bp_online/runEoH.py`
- Diagnosis: `python examples/bp_online/runEoH_diag.py`

**Where logs are stored**
- `examples/bp_online/results/diag_logs/`
