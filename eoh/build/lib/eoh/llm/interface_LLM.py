...
from ..llm.api_gemini import InterfaceGemini
    ...
    def __init__(self, api_endpoint, api_key, model_LLM,llm_use_local,llm_local_url, debug_mode):
        ...
        if self.llm_use_local:
            ...
            if self.api_key == None or self.api_endpoint ==None or self.api_key == 'xxx' or self.api_endpoint == 'xxx':
                ...
                exit()
            if self.api_endpoint in ["gemini", "google"] or "generativelanguage.googleapis.com" in self.api_endpoint:
                if self.api_key == None or self.api_key == "" or self.api_key == "xxx":
                    print(">> Stop with empty GEMINI_API_KEY !")
                    exit()

            if self.api_endpoint in ["gemini", "google"] or "generativelanguage.googleapis.com" in self.api_endpoint:
                self.interface_llm = InterfaceGemini(
                    self.api_key,
                    self.model_LLM,
                    self.debug_mode,
                )
